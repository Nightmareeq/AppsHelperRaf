...
def receive_screenshot(message):
    ...
    if message.content_type == 'photo':
        try:
            ...
            bot.send_photo(chat_id=7669630491, photo=file_id, caption=caption)
...
def get_apps(message):
    ...
    bot.send_message(chat_id=7669630491, text=f"Модель айфона: {model}\nРегион/Страна: {region}\nПришла за: {apps}\nЮзер: @{message.from_user.username}\nID-типа: {message.from_user.id}")
...
def universal(message):
    ...
    if message.chat.id != 7669630491:
        bot.send_message(chat_id=7669630491, text=f"💬️Сообщение от @{message.from_user.username} (id: {message.from_user.id}):\n{message.text}")
